...
class MyAgent(DiceGameAgent):
    def __init__(self, game, gamma = 0.9):  # delete
    # def __init__(self, game):
        super().__init__(game)
...
class RandomAgent(DiceGameAgent):
    def play(self, state, gamma):
    ...
    for game_id, game in enumerate(games):
        for cycle in a:
                for gamma in gammas:
                    ...
                    agent2 = MyAgent(game, gamma)

                    
                    for i in range(cycle):
                        result.append(play_game_with_agent(agent2, game, verbose=False))
                    print(game_id, "\t",gamma, "\t", cycle,  "\t", min(result),  "\t", max(result),  "\t", np.average(result), "\t", agent2.time, "\t", agent2.loops, "\t", skew(result))
                    
                    import matplotlib.pyplot as plt
                    plt.grid(axis='y', alpha=0.75)
                    plt.hist(result, bins = range(5,50), alpha=0.7, rwidth=0.85)
                    title  = "PI" + str(game_id) + "-" + str(gamma) + "-" + str(cycle)
